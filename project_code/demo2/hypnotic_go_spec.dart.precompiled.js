...
["", "hypnotic_go_spec.dart", , O, {
  ...
  main: [function() {
    ...
    $.particles = O.makeParticles(width, height);
    watch = new P.Stopwatch(null, null);
    watch.start$0(watch);
    C.Window_methods.get$animationFrame(window).then$1(O.loop$closure());
  }, "call$0", "main$closure", 0, 0, 0],
  makeParticles: function(width, height) {
    var particles, i, t1;
    particles = H.setRuntimeTypeInfo([], [U.Particle]);
    for (i = 0; i < 10000; ++i) {
      t1 = new U.Particle(null, null, null, null, null, null);
  ...
  loop: [function(delta) {
    var t1, particle, t2, t3, t4, t5, t6;
    if (J.$eq($.lastStamp, 0))
      $.lastStamp = delta;
    P.print(J.$sub$n(delta, $.lastStamp));
    $.lastStamp = delta;
    for (t1 = $.particles, t1 = new H.ListIterator(t1, t1.length, 0, null); t1.moveNext$0();) {
  ...
  render: function(particles) {
    var main, t1, width, height, mainCtxt, particle;
    main = document.querySelector("#main_canvas");
    t1 = J.getInterceptor$x(main);
    width = t1.get$width(main);
    height = t1.get$height(main);
    mainCtxt = t1.getContext$1(main, "2d");
    J.set$fillStyle$x(mainCtxt, "rgba(0, 0, 0, 0.05)");
    mainCtxt.fillRect(0, 0, width, height);
    mainCtxt.fillStyle = "rgba(255, 180, 180, 0.1)";
    for (t1 = new H.ListIterator(particles, particles.length, 0, null); t1.moveNext$0();) {
      particle = t1._current;
      mainCtxt.beginPath();
      mainCtxt.arc(particle.getX$0(), particle._y, 1, 0, 6.283185307179586, false);
      mainCtxt.closePath();
      mainCtxt.fill();
    }
  }
...
["", "particle.dart", , U, {
  "": "",
  Particle: {
    "": "Object;_x,_y,_vx,_vy,_width,_height",
    getX$0: function() {
      return this._x;
    },
    place$0: function() {
      var t1, t2, t3;
      this._x = $.get$Particle__rnd().nextDouble$0() * this._width;
      this._y = $.get$Particle__rnd().nextDouble$0() * this._height;
      t1 = $.Particle__impulse;
      t2 = $.get$Particle__rnd().nextDouble$0();
      t3 = $.Particle__impulse;
      this._vx = (t1 - t2 * (2 * t3)) * $.Particle__initImpulseSaler;
      this._vy = (t3 - $.get$Particle__rnd().nextDouble$0() * (2 * $.Particle__impulse)) * $.Particle__initImpulseSaler;
    },
    push$0: function() {
      var t1, t2, t3, t4, t5;
      t1 = this._vx;
      t2 = $.Particle__impulse;
      t3 = $.get$Particle__rnd().nextDouble$0();
      t4 = $.Particle__impulse;
      t5 = $.Particle__impulseScaler;
      if (typeof t1 !== "number")
        return t1.$add();
      ...
      t3 = $.get$Particle__rnd().nextDouble$0();
      t2 = $.Particle__impulse;
      t1 = $.Particle__impulseScaler;
      if (typeof t5 !== "number")
        return t5.$add();
      this._vy = t5 + (t4 - t3 * (2 * t2)) * t1;
    },
    static: {"": "Particle__rnd,Particle__impulse,Particle__initImpulseSaler,Particle__impulseScaler"}
  }
}],
]);
...
$.particles = null;
$.lastStamp = 0;
$.Particle__impulse = 0.005;
$.Particle__initImpulseSaler = 120;
$.Particle__impulseScaler = 6;
...
Isolate.$lazy($, "_rnd", "Particle__rnd", "get$Particle__rnd", function() {
...
function dart_precompiled($collectedClasses) {
  ...
  function Particle(_x, _y, _vx, _vy, _width, _height) {
    this._x = _x;
    this._y = _y;
    this._vx = _vx;
    this._vy = _vy;
