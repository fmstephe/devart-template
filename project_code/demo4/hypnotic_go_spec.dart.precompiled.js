...
["_interceptors", "dart:_interceptors", , J, {
  ...
  Interceptor: {
    ...
    "%": "ArrayBuffer|Blob|CanvasGradient|CanvasPattern|DOMError|File|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedNumberList"
  },
  JSBool: {
    "": "bool/Interceptor;",
    toString$0: function(receiver) {
...
["dart.async", "dart:async", , P, {
  ...
  _AsyncCompleter: {
    ...
    }, "completeError$1", "call$2", "call$1", "get$completeError", 2, 2, 14, 4],
    $as_Completer: null
  },
  _SyncCompleter: {
    "": "_Completer;future",
...
["", "hypnotic_go_spec.dart", , O, {
  "": "",
  main: [function() {
    var main, t1, width, height, mainCtxt, gospec;
    main = document.querySelector("#main_canvas");
    t1 = J.getInterceptor$x(main);
    width = t1.get$width(main);
    height = t1.get$height(main);
    mainCtxt = t1.getContext$1(main, "2d");
    J.set$fillStyle$x(mainCtxt, "rgba(0, 0, 0, 1)");
    mainCtxt.fillRect(0, 0, width, height);
    gospec = O.SpecText$();
    $.fishes = O.makeFish(width, height);
    O.cycleText(gospec);
    P.Timer_Timer$periodic(P.Duration$(0, 0, 0, 0, 0, 18), new O.main_closure(gospec));
    C.Window_methods.get$animationFrame(window).then$1(O.loop$closure());
  }, "call$0", "main$closure", 0, 0, 1],
  makeFish: function(width, height) {
    var Fishs, i, t1;
    Fishs = H.setRuntimeTypeInfo([], [O.Fish]);
    for (i = 0; i < 10000; ++i) {
      t1 = new O.Fish(null, null, null, null, null, null, null, null);
  ...
  cycleText: function(gospec) {
    var txt, t1, width, textCtxt;
    P.print("Blanked");
    txt = document.querySelector("#text_canvas");
    t1 = J.getInterceptor$x(txt);
    $.height = t1.get$height(txt);
    width = t1.get$width(txt);
    textCtxt = t1.getContext$1(txt, "2d");
    J.set$fillStyle$x(textCtxt, "rgba(0, 0, 0, 1)");
    textCtxt.fillRect(0, 0, width, $.height);
    $.data = J.get$data$x(P.convertNativeToDart_ImageData(textCtxt.getImageData(0, 0, width, $.height)));
    $.clearLetter = true;
    P.Future_Future$delayed(P.Duration$(0, 0, 0, 0, 0, 4), new O.cycleText_closure(gospec));
  },
  nextLetter: function(gospec) {
    var $char, txt, t1, width, textCtxt;
    $char = gospec.nextChar$0();
    if ($char == null) {
      P.Future_Future$delayed(P.Duration$(0, 0, 0, 0, 0, 1), new O.nextLetter_closure(gospec));
      return;
    }
    $.clearLetter = false;
    P.print("New Char");
    ...
    width = t1.get$width(txt);
    textCtxt = t1.getContext$1(txt, "2d");
    J.set$fillStyle$x(textCtxt, "rgba(255, 255, 255, 1)");
    textCtxt.fillRect(0, 0, width, $.height);
    textCtxt.fillStyle = "rgba(0, 0, 0, 1)";
    textCtxt.font = "bold " + J.toString$0($.height) + "px sans-serif";
    ...
    if (typeof t1 !== "number")
      return t1.$div();
    textCtxt.fillText($char, width / 2, t1 / 2 - 5);
    $.data = J.get$data$x(P.convertNativeToDart_ImageData(textCtxt.getImageData(0, 0, width, $.height)));
  },
  loop: [function(delta) {
    var t1, fish, t2, t3, t4, idx, t5, t6, t7, t8;
    if (J.$eq($.lastStamp, 0))
      $.lastStamp = delta;
    $.lastStamp = delta;
    for (t1 = $.fishes, t1 = new H.ListIterator(t1, t1.length, 0, null); t1.moveNext$0();) {
      fish = t1._current;
      if ($.clearLetter)
        fish.speedUp2D$0();
      else if ($.get$rnd().nextDouble$0() > 0.7) {
        t2 = fish.getX$0();
        t3 = fish._y;
        t3.toString;
        t3 = C.JSNumber_methods.toInt$0(Math.floor(t3));
        t4 = $.height;
        if (typeof t4 !== "number")
          return H.iae(t4);
        t2.toString;
        idx = (t3 * t4 + C.JSNumber_methods.toInt$0(Math.floor(t2))) * 4;
        t2 = $.data;
        t3 = t2.length;
        if (idx < t3) {
          if (idx < 0)
    ...
    C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(O.loop$closure()));
  }, "call$1", "loop$closure", 2, 0, 9],
  renderMain: function(fishes) {
    var main, t1, width, height, mainCtxt, fish;
    main = document.querySelector("#main_canvas");
    t1 = J.getInterceptor$x(main);
    width = t1.get$width(main);
    height = t1.get$height(main);
    mainCtxt = t1.getContext$1(main, "2d");
    J.set$fillStyle$x(mainCtxt, "rgba(0, 0, 0, 0.2)");
    mainCtxt.fillRect(0, 0, width, height);
    mainCtxt.fillStyle = "rgba(255, 180, 180, 0.1)";
    for (t1 = new H.ListIterator(fishes, fishes.length, 0, null); t1.moveNext$0();) {
      fish = t1._current;
      mainCtxt.beginPath();
      mainCtxt.arc(fish.getX$0(), fish._y, fish._z, 0, 6.283185307179586, false);
      mainCtxt.closePath();
      mainCtxt.fill();
    }
  },
  main_closure: {
    ...
    call$1: function(timer) {
      return O.cycleText(this.gospec_0);
    }
  },
  cycleText_closure: {
    "": "Closure:10;gospec_0",
    call$0: function() {
      return O.nextLetter(this.gospec_0);
    }
  },
  nextLetter_closure: {
    "": "Closure:10;gospec_0",
  ...
  Fish: {
    "": "Object;_x,_y,_z,_vx,_vy,_vz,_width,_height",
    getX$0: function() {
      return this._x;
    },
    place$0: function() {
      var t1 = $.get$Fish__rnd();
      this._x = t1.nextDouble$0() * this._width;
      this._y = t1.nextDouble$0() * this._height;
      this._z = t1.nextDouble$0() * 1.7 + 0.3;
      this._vx = 0;
      this._vy = 0;
      this._vz = 0;
      this.push$3((0.003 - t1.nextDouble$0() * 0.006) * 60, (0.003 - t1.nextDouble$0() * 0.006) * 60, (0.003 - t1.nextDouble$0() * 0.006) * 6);
    },
    pushRandom$0: function() {
      var t1 = $.get$Fish__rnd();
      this.push$3((0.003 - t1.nextDouble$0() * 0.006) * 6, (0.003 - t1.nextDouble$0() * 0.006) * 6, (0.003 - t1.nextDouble$0() * 0.006) * 6);
    },
    push$3: function(px, py, pz) {
      var t1, t2;
      t1 = this._vx;
      if (typeof t1 !== "number")
        return t1.$add();
      ...
      if (typeof t2 !== "number")
        return t2.$add();
      this._vz = t2 + pz;
      this._vx = P.max(P.min(t1, 0.18), -0.18);
      this._vy = P.max(P.min(this._vy, 0.18), -0.18);
      this._vz = P.max(P.min(this._vz, 0.018000000000000002), -0.018000000000000002);
    },
    speedUp2D$0: function() {
      var t1, t2, t3;
      t1 = this._vx;
      if (typeof t1 !== "number")
        return t1.$div();
      ...
      if (typeof t3 !== "number")
        return t3.$div();
      this.push$3(t1 / 4, t2 / 4, t3 / 4);
    },
    static: {"": "Fish__rnd,Fish__impulse,Fish__initImpulseSaler,Fish__initZImpulseSaler,Fish__impulseScaler,Fish_maxNear,Fish_maxFar,Fish_maxV2D,Fish_maxVZ,Fish_changeDivisor"}
  },
  SpecText: {
    "": "Object;_text,_idx",
    nextChar$0: function() {
      ...
      if (t1 == null)
        return;
      idx = C.JSInt_methods.$mod(this._idx, t1.length);
      c = J.substring$2$s(t1, idx, idx + 1);
      this._idx = this._idx + 1;
      return c.toUpperCase();
    },
    SpecText$0: function() {
      var t1 = P.Uri_base();
      W.HttpRequest_request(t1.get$origin(t1) + "/hypnotic_go_spec.dart", null, null, null, null, null, null, null).then$1(new O.SpecText_closure(this));
    },
    static: {SpecText$: function() {
        var t1 = new O.SpecText(null, 0);
        t1.SpecText$0();
        return t1;
      }}
  },
  SpecText_closure: {
    ...
    call$1: function(request) {
      var t1 = J.get$responseText$x(request);
      this.this_0._text = t1;
      return t1;
    }
  }
},
...
Isolate.$lazy($, "_rnd", "Fish__rnd", "get$Fish__rnd", function() {
  return C.C__JSRandom;
});
// Native classes
...
function dart_precompiled($collectedClasses) {
  ...
  function main_closure(gospec_0) {
    this.gospec_0 = gospec_0;
  }
  ...
  function cycleText_closure(gospec_0) {
    this.gospec_0 = gospec_0;
  }
  cycleText_closure.builtin$cls = "cycleText_closure";
  if (!"name" in cycleText_closure)
    cycleText_closure.name = "cycleText_closure";
  $desc = $collectedClasses.cycleText_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  cycleText_closure.prototype = $desc;
  function nextLetter_closure(gospec_0) {
    this.gospec_0 = gospec_0;
  }
  nextLetter_closure.builtin$cls = "nextLetter_closure";
  if (!"name" in nextLetter_closure)
    nextLetter_closure.name = "nextLetter_closure";
  $desc = $collectedClasses.nextLetter_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  nextLetter_closure.prototype = $desc;
  function Fish(_x, _y, _z, _vx, _vy, _vz, _width, _height) {
    this._x = _x;
    this._y = _y;
    this._z = _z;
    this._vx = _vx;
